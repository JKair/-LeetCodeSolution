...
1. Count Primes:
思路并不难，就是排除那些不是素数的就好了

1. Power of Two:
检查是不是2的n次幂，最原始的思路就是直接破解了，然后还有一个思路就是2的n次幂转换成2进制是会变成1000....总之只有一个1，然后2的n次幂减1就会变成1111....，全部是1，按位与，第一位补0就会变成只有0了，利用这个就能快速解答

1. Move Zeros:
移动0到最后面，思路有好几种吧，这里说交换位置的，主要是需要一个标志比较，下标的大小比较。注意这两点就很容易了